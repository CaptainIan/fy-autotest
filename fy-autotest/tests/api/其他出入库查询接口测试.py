...
对应 curl:
curl 'https://kdsa1-api-uat.wilmar.cn/api/inventory/otherstock/_search?pagesize=10&pagenum=1' \
...
class TestCase(TestCase):
    ...
    def test_otherstock_search_p0(self):
        """
        TC-P0: 其他出入库查询 - 正常场景（固定日期范围）
        """
        ...
        response = requests.post(
            "/api/inventory/otherstock/_search",
        ...
        if items:
            ...
            assert "BillStatus" in first_item, "缺少 BillStatus 字段"
        
        logger.log_info("✅ 其他出入库查询 P0 测试通过")
    
    def test_otherstock_search_different_page(self):
        """
        TC-P0: 其他出入库查询 - 分页查询
        """
        payload = {
    ...
    def test_otherstock_search_by_status(self):
        """
        TC-P0: 其他出入库查询 - 按单据状态筛选
        """
        # 只查询状态为 "2"（已审核）的单据
        payload = {
            "BillTypeSubIds": [],
    ...
    def test_otherstock_search_by_bill_type(self):
        """
        TC-P0: 其他出入库查询 - 按单据类型筛选
        """
        # 查询指定单据类型
        payload = {
            "BillTypeSubIds": [1],  # 指定单据类型
            "BillStatus": ["2", "3", "7", "14", "15", "16", "17"],
            "MoveTypeSubIds": [],
            "BeginDoBillDate": "2026-01-13",
            "EndDoBillDate": "2026-04-13",
        ...
        logger.log_info(f"按单据类型筛选查询，返回数据条数: {len(items)}")
        logger.log_info("✅ 按单据类型筛选测试通过")
    
    def test_otherstock_search_invalid_token(self):
        """
        TC-P1: 其他出入库查询 - 无效 Token 场景
        """
        # 临时清除 Token
    ...
    def test_otherstock_search_empty_date(self):
        """
        TC-P1: 其他出入库查询 - 空日期范围（边界场景）
        """
        payload = {
            "BillTypeSubIds": [],
            "BillStatus": ["2", "3", "7", "14", "15", "16", "17"],
    ...
    def test_otherstock_search_large_pagesize(self):
        """
        TC-P1: 其他出入库查询 - 大分页边界场景
        """
        payload = {
            "BillTypeSubIds": [],
            "BillStatus": ["2", "3", "7", "14", "15", "16", "17"],
